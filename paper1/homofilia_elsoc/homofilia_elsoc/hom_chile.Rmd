---
title: "Calculo de homofilia COES"
...
coes$F6[coes$F6 == 7 | coes$F6 == 8 ] <- 'center-right'
...
for (i in 1:5){
  eval(parse(text=paste(
    'coes$alter_relacion',i,'[coes$alter_relacion',i,'==1  & !is.na(coes$alter_relacion',i,')]<-\'fam\' ;',
    ...
    'coes$alter_relacion',i,'[coes$alter_relacion',i,'==11 & !is.na(coes$alter_relacion',i,')]<-\'nofam\' ',
    sep='')))
}
```
...
hom_2014<-egltable(c("sexo_dist", "edad_dist", "educ_dist", "relig_dist", "barrio_dist"),
                      data = homoL_2014, g="case", strict=T)
show_html(hom_2014)
```
...
for (i in 1:5){
  eval(parse(text=paste(
    'el$r13_ideol_0',i,'[el$r13_ideol_0',i,'==1 & !is.na(el$r13_ideol_0',i,')]<-\'rightwinger\' ;',
    ...
    'el$r13_ideol_0',i,'[el$r13_ideol_0',i,'==5 & !is.na(el$r13_ideol_0',i,')]<-\'leftwinger\' ', 
    sep='')))
}
...
#rownames(hom_abs_2017)<-c("Distancia observada por sexo",
#                          "Distancia esperada por chance por sexo",
#                          "Distancia observada por educación",
#                          "Distancia esperada por chance por educación",
#                          "Distancia observada por edad",
...
# Descriptivos (Media y error estandar por bootstrap)
## Sexo distancia (observada y simulada)
```{r}
...
#rownames(hom_abs_2019)<-c("Distancia observada por sexo",
#                          "Distancia esperada por chance por sexo",
#                          "Distancia observada por educación",
#                          "Distancia esperada por chance por educación",
#                          "Distancia observada por edad",
...
hom_abs %>%
  ...
  footnote(number = c("Los errores estándar se calculan a partir de muestras bootstrap para el nivel observado de homofilia y utilizando un diseño de encuesta complejo para el nivel esperado por chance; "))
...
hom_2019<-egltable(c("sexo_dist", "edad_dist", "educ_dist", "relig_dist", "barrio_dist"),
                      data = homoL_2019, g="case", strict=T)
hom_2019<-as.data.frame(hom_2019)

hom_2019 %>%
  kbl(caption = "Homofilia absoluta 2019") %>%
    kable_classic("hover", full_width = F)
```


La tabla anterior muestra pruebas t por grupo caso / control para el año 2019.

Esta tabla es la misma de Smith et al. (2014). Describe la media de la distancia sociodemográfica por parámetro. Como alternativa, el valor TRUE del argumento `strict` de la función `egltable`, muestra los porcentajes por categoría de la variable dummy.

# Descriptivos gral.

## Homofilia absoluta observada por año

### Juntamos la data de los 3 años
```{r}
hom_abs<-rbind(homoL_2014,homoL_2017,homoL_2019)
```
...
```{r,results='asis',message=FALSE}
hom_abs_obs<-egltable(c("sexo_dist", "edad_dist", "educ_dist", "relig_dist", "barrio_dist"),
                      data = hom_abs_obs, g="año", strict=T)
show_html(hom_abs_obs)
```

## Homofilia absoluta simulada por año
```{r}
hom_abs_sim<-hom_abs%>%
  dplyr::filter(case==0)
```
...
```{r, results='asis',message=FALSE}
hom_abs_sim<-egltable(c("sexo_dist", "edad_dist", "educ_dist", "relig_dist", "barrio_dist"),
                      data = hom_abs_sim, g="año", strict=T)
show_html(hom_abs_sim)
```
...
```{r}
hom_rel<-hom_rel %>% drop_na(relig)
hom_rel<-hom_rel %>% drop_na(ideol)
hom_rel<-hom_rel %>% drop_na(sexo)
hom_rel<-hom_rel %>% drop_na(educ)
#hom_rel<-hom_rel %>% drop_na(ego_m_educ)

hom_rel$edad  <-as.numeric(hom_rel$edad)
hom_rel$edad1 <-as.numeric(hom_rel$edad1)
hom_rel$edad2 <-as.numeric(hom_rel$edad2)
hom_rel$edad3 <-as.numeric(hom_rel$edad3)
hom_rel$edad4 <-as.numeric(hom_rel$edad4)
hom_rel$edad5 <-as.numeric(hom_rel$edad5)
#hom_rel$machismo<-as.numeric(hom_rel$machismo)
#hom_rel$cal_entorno<-as.numeric(hom_rel$cal_entorno)
```

```{r}
save(hom_rel, file = "hom_rel.RData")
```
...
var.name.characs=c("educ", "relig", "sexo", "edad", "barrio", "ideol") 
...
```{r}
hom_rel_2014<-hom_rel[hom_rel$año == '2014',]
hom_rel_2014<-as.matrix(hom_rel_2014)
hom_rel_2014<-as.data.frame(hom_rel_2014)

hom_rel_2014$edad  <-as.numeric(hom_rel_2014$edad)
hom_rel_2014$edad1 <-as.numeric(hom_rel_2014$edad1)
hom_rel_2014$edad2 <-as.numeric(hom_rel_2014$edad2)
hom_rel_2014$edad3 <-as.numeric(hom_rel_2014$edad3)
hom_rel_2014$edad4 <-as.numeric(hom_rel_2014$edad4)
hom_rel_2014$edad5 <-as.numeric(hom_rel_2014$edad5)
...
#
#```{r}
#formula_ego_mix<- as.formula(~ nodemix("sexo") +
#                               nodemix("educ") + 
#                               nodemix("relig") + 
#                               nodemix("ideol")) 
#```
#
#```{r}
#modelo2<-egonet_case_control_model(formula=formula_ego_mix,
#                                   ego_data=hom_rel,
#                                   var.name.degree=var.name.degree, 
#                                   var.name.characs=var.name.characs,
#                                   var.name.characs.alter=var.name.characs.alter, 
