...
names(ego_network_enacoes_2014)<- c('id',
...
#
#ego_network_enacoes_2014$ideol  <-as.character(ego_network_enacoes_2014$ideol)
#ego_network_enacoes_2014$ideol1 <-as.character(ego_network_enacoes_2014$ideol1)
#ego_network_enacoes_2014$ideol2 <-as.character(ego_network_enacoes_2014$ideol2)
#ego_network_enacoes_2014$ideol3 <-as.character(ego_network_enacoes_2014$ideol3)
#ego_network_enacoes_2014$ideol4 <-as.character(ego_network_enacoes_2014$ideol4)
#ego_network_enacoes_2014$ideol5 <-as.character(ego_network_enacoes_2014$ideol5)
#
...
ego_network_enacoes_2014<-ego_network_enacoes_2014 %>% drop_na(ideol)
#ego_network_enacoes_2014<-ego_network_enacoes_2014 %>% drop_na(edad)
...
```{r}
formula_ego_enacoes<- as.formula(~ nodematch("sexo", diff=TRUE) + 
                                   nodefactor("sexo")+
...
bic=egonet_case_control_output$fit["bic"]
aic=egonet_case_control_output$fit["aic"]
head(coefs)
#rm(mean)
...
std_mean(coefs_sample$nodematch.ideol.rightwinger)
```
...
ego_network_enacoes_2014_ego<-ego_network_enacoes_2014%>%
  dplyr::select(id,sexo,edad,educ,relig,ideol,tamred,POND_MUESTRAL)

ego_network_enacoes_2014_alter<-ego_network_enacoes_2014%>%
  dplyr::select(id,
...
# reshape
ego_network_enacoes_2014_alter_long<-long_panel(ego_network_enacoes_2014_alter, 
                                                prefix="", 
                                                begin=1, 
                                                end=5, 
                                                label_location="end")
...
# filter
ego_network_enacoes_2014_alter_long_nofam<-ego_network_enacoes_2014_alter_long%>%
  dplyr::filter(relacion =="nofam")

# to wide
ego_network_enacoes_2014_alter_nofam<-widen_panel(ego_network_enacoes_2014_alter_long_nofam, separator = "") # problemas con la función de panelr
ego_network_enacoes_2014_alter_nofam<-ego_network_enacoes_2014_alter_nofam%>%
  select(-contains("."))


ego_network_enacoes_2014_alter_nofam<-as.matrix(ego_network_enacoes_2014_alter_nofam)
ego_network_enacoes_2014_alter_nofam<-as.data.frame(ego_network_enacoes_2014_alter_nofam)
#ego_network_enacoes_2014_alter_nofam$id<-NULL

# join
ego_network_enacoes_2014_nofam<-dplyr::left_join(ego_network_enacoes_2014_alter_nofam, 
                                     ego_network_enacoes_2014_ego, 
                                     by="id")%>%
  dplyr::select(id,tamred,
                sexo,sexo1,sexo2,sexo3,sexo4,sexo5,
                edad,edad1,edad2,edad3,edad4,edad5,
                educ,educ1,educ2,educ3,educ4,educ5,
                relig,relig1,relig2,relig3,relig4,relig5,
                ...
                POND_MUESTRAL)
# to numeric
ego_network_enacoes_2014_nofam$edad  <-as.numeric(ego_network_enacoes_2014_nofam$edad)
ego_network_enacoes_2014_nofam$edad1 <-as.numeric(ego_network_enacoes_2014_nofam$edad1)
ego_network_enacoes_2014_nofam$edad2 <-as.numeric(ego_network_enacoes_2014_nofam$edad2)
ego_network_enacoes_2014_nofam$edad3 <-as.numeric(ego_network_enacoes_2014_nofam$edad3)
ego_network_enacoes_2014_nofam$edad4 <-as.numeric(ego_network_enacoes_2014_nofam$edad4)
ego_network_enacoes_2014_nofam$edad5 <-as.numeric(ego_network_enacoes_2014_nofam$edad5)
...
egonet_case_control_output_nf<-egonet_case_control_model(formula=formula_ego_enacoes,
                                                     ego_data=ego_network_enacoes_2014_nofam,
                                                     var.name.degree=var.name.degree, 
                                                     var.name.characs=var.name.characs,
                                                     var.name.characs.alter=var.name.characs.alter, 
                                                     case.control.type="weighted.random.matching", 
...
```{r}
names(egonet_case_control_output_nf)
coefs=egonet_case_control_output_nf$coefs
bic=egonet_case_control_output_nf$fit["bic"]
aic=egonet_case_control_output_nf$fit["aic"]
head(coefs)
#rm(mean)
